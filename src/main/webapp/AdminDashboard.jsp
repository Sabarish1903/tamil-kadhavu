		...
		<table>
			...
			<tbody>
				...
				try {
					...
					// Using LOWER() ensures we don't miss users due to Capitalization issues
					...
					while (rs.next()) {
						...
						// NULL GUARD: If experience is empty in DB, show "New" instead of crashing
				...
				<tr>
					...
					<td><%=teacherEmail%></td>
					<td><%=exp%></td>
					<td><a href="ApproveUserServlet?email=<%=teacherEmail%>"
						class="approve-btn"> Approve </a></td>
				</tr>
				<%
				}
				if (!found) {
				%>
				<tr>
					<td colspan="4" class="no-data">தற்போது நிலுவையில் உள்ள
						கோரிக்கைகள் எதுவுமில்லை (No pending approvals).</td>
				</tr>
				<%
				}
				} catch (Exception e) {
